# Central Bank of Bahrain Exchange Rate API client

Official **Central Bank of Bahrain** (Bahrain) daily exchange rates in Node.js / TypeScript — 33 currencies against the BHD, with history back to 2016. Zero dependencies, works in Node 18+, Bun, Deno, and edge runtimes (uses global `fetch`).
...
Powered by [AllRatesToday](https://allratestoday.com/central-bank-rates-api/cbb/). Get a free API key at [allratestoday.com/register](https://allratestoday.com/register) — no credit card required.
...
console.log(pair.rate, pair.rate_date); // e.g. USD -> BHD on the bank's own date
...
If Central Bank of Bahrain does not print a pair directly, the API resolves it from the bank's table (inverse, or a cross rate via BHD) and flags it `derived: true` with the `method` — so official and computed values are never confused.
