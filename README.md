# Central Bank of Bahrain Exchange Rates API — cbb-exchange-rate

[![npm version](https://img.shields.io/npm/v/cbb-exchange-rate.svg)](https://www.npmjs.com/package/cbb-exchange-rate)
...
- 🏛️ **Official published rates** — Central Bank of Bahrain's own table, with the publisher's own `rate_date` on every response
...
> **Official rate, not mid-market:** every value here is a number Central Bank of Bahrain itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Central Bank of Bahrain table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/cbb?source=USD&target=BHD"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/cbb').then((x) => x.json());
...
```bash
yarn add cbb-exchange-rate
```

```bash
pnpm add cbb-exchange-rate
```

Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/cbb-exchange-rate`](https://www.npmjs.com/package/@allratestoday/cbb-exchange-rate) — same code, same versions.
...
console.log(pair.rate, pair.rate_date); // the official Central Bank of Bahrain rate, on the central bank's own date
```
...
```js
import { getLatestRates } from 'cbb-exchange-rate';
...
```js
import { getRatesForDate } from 'cbb-exchange-rate';

const day = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...' });
// Optionally narrow to one pair:
const one = await getRatesForDate('2026-06-30', { apiKey: 'art_live_...', source: 'USD', target: 'BHD' });
```
...
Central Bank of Bahrain currently publishes rates covering **33 currencies** against the BHD (as of the latest table):

🇦🇪 `AED` · `ANG` · 🇦🇺 `AUD` · 🇧🇩 `BDT` · 🇨🇦 `CAD` · 🇨🇭 `CHF` · 🇨🇳 `CNY` · 🇪🇬 `EGP` · 🇪🇺 `EUR` · 🇬🇧 `GBP` · 🇭🇰 `HKD` · 🇮🇩 `IDR` · 🇮🇱 `ILS` · 🇮🇳 `INR` · 🇯🇴 `JOD` · 🇯🇵 `JPY` · 🇰🇼 `KWD` · 🇱🇧 `LBP` · 🇱🇰 `LKR` · 🇲🇦 `MAD` · 🇳🇴 `NOK` · 🇳🇵 `NPR` · 🇳🇿 `NZD` · 🇴🇲 `OMR` · 🇵🇭 `PHP` · 🇵🇰 `PKR` · 🇶🇦 `QAR` · 🇸🇦 `SAR` · 🇸🇬 `SGD` · 🇹🇭 `THB` · 🇹🇳 `TND` · 🇹🇷 `TRY` · 🇺🇸 `USD`
...
- Publisher's own page: [Facilities, interest and exchange rates](https://www.cbb.gov.bh/facilities-interest-rates/) · [www.cbb.gov.bh](https://www.cbb.gov.bh)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Central Bank of Bahrain rates page](https://allratestoday.com/central-bank-rates-api/cbb/)
...
- Central Bank of Bahrain quotes **BHD per 1 unit of foreign currency** (e.g. `base: "USD", quote: "BHD"` means BHD per one US dollar).
...
Loading the official Central Bank of Bahrain rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/cbb/latest?format=xml&api_key=art_live_..."
```
...
If Central Bank of Bahrain does not print a pair directly, the API resolves it from the central bank's own table and says so — official and computed values are never confused:

| `method` | `derived` | Meaning |
| --- | --- | --- |
| `published` | `false` | The central bank printed this pair directly |
| `inverse` | `true` | Computed as 1 ÷ the published opposite direction |
| `cross` | `true` | Computed via BHD from two published rates |
...
| `404` | Pair or date range not covered by Central Bank of Bahrain |
...
```ts
import type { LatestRates, PairRate, DatedRates, RateEntry, HistoryQuery, RequestOptions } from 'cbb-exchange-rate';
```

## 📦 CommonJS

```javascript
const { getRate } = require('cbb-exchange-rate');

getRate('USD', 'BHD', { apiKey: 'art_live_...' }).then((pair) => console.log(pair.rate));
```
...
- Hugging Face: [AllRates/central-bank-exchange-rates](https://huggingface.co/datasets/AllRates/central-bank-exchange-rates) — one CSV per institution (`rates/cbb.csv`)
- Kaggle: [allratestoday/central-bank-exchange-rates](https://www.kaggle.com/datasets/allratestoday/central-bank-exchange-rates)
- CDN JSON: `https://cdn.jsdelivr.net/gh/AllRates-Today/central-bank-exchange-rates@main/data/cbb/latest.json`

## 🔗 Links

- [Central Bank of Bahrain rates page](https://allratestoday.com/central-bank-rates-api/cbb/) — live table, publication cadence, FAQ
- [All central bank sources](https://allratestoday.com/central-bank-rates-api/)
- [Package docs on the site](https://allratestoday.com/docs/sdk/cbb-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
...
- [GitHub](https://github.com/AllRates-Today/cbb-exchange-rate)
